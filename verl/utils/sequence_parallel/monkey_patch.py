...
from .ring import ring_flash_attn_varlen_func, zigzag_ring_flash_attn_varlen_func
...
def _ulysses_attn_varlen_func(
    ...
    # print(f"attn_output_unpad.shape {attn_output_unpad.shape}")  # attn_output_unpad.shape torch.Size([21124, 7, 128])
    attn_output = pad_input(attn_output_unpad, indices_q, batch_size, query_length)

    # print(f"attn_output.shape {attn_output.shape}")  # attn_output.shape torch.Size([2, 10562, 7, 128])
...
def _flash_attention_forward(
    ...
    # print(f"Patched, {attention_mask.shape}", flush=True)

    seqlens_in_batch = torch.sum(attention_mask, dim=1)

    attn_output = UlyssesAttention(_ulysses_attn_varlen_func, get_ulysses_sequence_parallel_group())(
        query_states,
        key_states,
        value_states,
        # query_length,
    ...
    # print(f"in patch, attn_output.shape {attn_output.shape}")

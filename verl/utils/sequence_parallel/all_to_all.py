...
def all_to_all_4D(input: torch.tensor, scatter_idx: int = 2, gather_idx: int = 1, group=None) -> torch.tensor:
    ...
    if scatter_idx == 2 and gather_idx == 1:
        ...
        # print(
        #     f"ulysses_seq_len {ulysses_seq_len}, shard_seqlen {shard_seqlen}, seq_world_size {seq_world_size}, input.shape {input.shape}"
        # )
        ...
        # print(f"output.shape {output.shape}")  # ([10568, 2, 7, 128])
        # print(f"type(max_global_length): {type(max_global_length)}, max_global_length: {max_global_length}")
